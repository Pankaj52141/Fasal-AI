...
export async function POST(request: NextRequest) {
  try {
    ...
    const tmpPath = await writeTempFile(file, "image")

    const scriptPath = path.join(process.cwd(), "models", "predict_image.py")
    const result = await runPython(scriptPath, { image_path: tmpPath })
...
// Removed duplicated legacy block with conflicting imports and POST handler.
